...
public class BoardHash {
    ...
    public int useHeight(int index) {
        return hashTableUseHeight[index];
    }

    public int ignoreHeight(int index) {
    ...
    public synchronized void clearHash() {
        for (int i = 0; i < maxHashEntries; i++) {
            this.hashTableUseHeight[i * HashIndex.getNumHashFields() + HashIndex.FLAG.getIndex()] = HashValueType.EMPTY.getIndex();
            ...
            this.hashTableIgnoreHeight[i * HashIndex.getNumHashFields() + HashIndex.FLAG.getIndex()] = HashValueType.EMPTY.getIndex();
    ...
    public synchronized void setHashTable() {
        if (maxHashEntries != lastHashSizeCreated) {
            ...
            for (int i = 0; i < maxHashEntries; i++) {
                this.hashTableUseHeight[i * HashIndex.getNumHashFields() + HashIndex.FLAG.getIndex()] = HashValueType.EMPTY.getIndex();
                this.hashTableUseHeight[i * HashIndex.getNumHashFields() + HashIndex.HASHENTRY_HEIGHT.getIndex()] = Hash.DEFAULT_SEARCH_HASH_HEIGHT.getValue();
                this.hashTableUseHeight[i * HashIndex.getNumHashFields() + HashIndex.VERSION.getIndex()] = 1;
                this.hashTableIgnoreHeight[i * HashIndex.getNumHashFields() + HashIndex.FLAG.getIndex()] = HashValueType.EMPTY.getIndex();
                this.hashTableIgnoreHeight[i * HashIndex.getNumHashFields() + HashIndex.HASHENTRY_HEIGHT.getIndex()] = Hash.DEFAULT_SEARCH_HASH_HEIGHT.getValue();
                this.hashTableIgnoreHeight[i * HashIndex.getNumHashFields() + HashIndex.VERSION.getIndex()] = 1;
            }
        }
    }
    ...
    public void storeHashMove(int move, EngineBoard board, int score, byte flag, int height) {
        ...
        if (height >= this.hashTableUseHeight[hashIndex + HashIndex.HASHENTRY_HEIGHT.getIndex()] || hashTableVersion > this.hashTableUseHeight[hashIndex + HashIndex.VERSION.getIndex()]) {
            if (hashTableVersion == this.hashTableUseHeight[hashIndex + HashIndex.VERSION.getIndex()]) {
    ...
    private void copyEntryFromUseHeightToIgnoreHeightTable(int hashIndex) {
        this.hashTableIgnoreHeight[hashIndex + HashIndex.MOVE.getIndex()] = this.hashTableUseHeight[hashIndex + HashIndex.MOVE.getIndex()];
        this.hashTableIgnoreHeight[hashIndex + HashIndex.SCORE.getIndex()] = this.hashTableUseHeight[hashIndex + HashIndex.SCORE.getIndex()];
        this.hashTableIgnoreHeight[hashIndex + HashIndex.FLAG.getIndex()] = this.hashTableUseHeight[hashIndex + HashIndex.FLAG.getIndex()];
        ...
        this.hashTableIgnoreHeight[hashIndex + HashIndex.VERSION.getIndex()] = this.hashTableUseHeight[hashIndex + HashIndex.VERSION.getIndex()];
    }
    ...
    private void storeMoveInHashTable(int move, EngineBoard board, int score, byte flag, int height, int hashIndex, int[] hashTableUseHeight) {
        hashTableUseHeight[hashIndex + HashIndex.MOVE.getIndex()] = move;
        hashTableUseHeight[hashIndex + HashIndex.SCORE.getIndex()] = score;
        hashTableUseHeight[hashIndex + HashIndex.FLAG.getIndex()] = flag;
        ...
        hashTableUseHeight[hashIndex + HashIndex.VERSION.getIndex()] = hashTableVersion;
    }
