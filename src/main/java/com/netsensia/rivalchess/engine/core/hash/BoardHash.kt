...
class BoardHash {
    ...
    fun clearHash() {
        for (i in 0 until maxHashEntries) {
            hashTableUseHeight[i * NUM_HASH_FIELDS + HASHENTRY_FLAG] = HashValueType.EMPTY.index
            hashTableUseHeight[i * NUM_HASH_FIELDS + HASHENTRY_HEIGHT] = DEFAULT_SEARCH_HASH_HEIGHT
            hashTableIgnoreHeight[i * NUM_HASH_FIELDS + HASHENTRY_FLAG] = HashValueType.EMPTY.index
    ...
    fun storeHashMove(move: Int, board: EngineBoard, score: Int, flag: Byte, height: Int) {
        val hashIndex = (board.trackedBoardHashCode() % maxHashEntries).toInt() * NUM_HASH_FIELDS
    ...
    private fun storeMoveInHashTable(move: Int, board: EngineBoard, score: Int, flag: Byte, height: Int, hashIndex: Int, hashTableUseHeight: IntArray) {
        ...
        hashTableUseHeight[hashIndex + HASHENTRY_64BIT1] = (board.trackedBoardHashCode() ushr 32).toInt()
        hashTableUseHeight[hashIndex + HASHENTRY_64BIT2] = (board.trackedBoardHashCode() and LOW32).toInt()
