...
{
	protected static final long[] occupancyMaskRook = {
	...
	protected static final long[] occupancyMaskBishop = {
	...
	protected static long[] magicNumberRook = {
	...
	protected static long magicNumberBishop[] = {
	...
	protected static int magicNumberShiftsRook[] = {
	...
	protected static int magicNumberShiftsBishop[] = {
	...
	protected long magicMovesRook[][];
	protected long magicMovesBishop[][];
		...
		for (bitRef=0; bitRef<=63; bitRef++)
		{
			...
			for (i=0; i<variations; i++)
			{
				...
				if (isRook)
				{
					...
					for (j=bitRef+8; j<=63; j+=8) { validMoves |= (1L << j); if ((occupancyVariation[i] & (1L << j)) != 0) break; }
					for (j=bitRef-8; j>=0; j-=8) { validMoves |= (1L << j); if ((occupancyVariation[i] & (1L << j)) != 0) break; }
					for (j=bitRef+1; j%8!=0; j++) { validMoves |= (1L << j); if ((occupancyVariation[i] & (1L << j)) != 0) break; }
					for (j=bitRef-1; j%8!=7 && j>=0; j--) { validMoves |= (1L << j); if ((occupancyVariation[i] & (1L << j)) != 0) break; }
				...
				else
				{
					...
					for (j=bitRef+9; j%8!=0 && j<=63; j+=9) { validMoves |= (1L << j); if ((occupancyVariation[i] & (1L << j)) != 0) break; }
					for (j=bitRef-9; j%8!=7 && j>=0; j-=9) { validMoves |= (1L << j); if ((occupancyVariation[i] & (1L << j)) != 0) break; }
