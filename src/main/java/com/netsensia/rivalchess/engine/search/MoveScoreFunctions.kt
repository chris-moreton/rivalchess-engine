...
fun scorePieceSquareValues(board: EngineBoard, fromSquare: Int, toSquare: Int): Int {
    ...
    return when (piece) {
        ...
        BITBOARD_WN, BITBOARD_BN -> linearScale(
                if (board.mover == Colour.WHITE) board.blackPieceValues + board.pawnValuesCalculated(BITBOARD_BP) else board.whitePieceValues + board.pawnValuesCalculated(BITBOARD_WP),
