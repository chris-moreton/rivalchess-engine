...
fun isHeightHashTableEntryValid(depthRemaining: Int, boardHash: BoardHash, hashIndex: Int) =
        if (boardHash.useHeight(hashIndex + HASHENTRY_HEIGHT) >= depthRemaining &&
                boardHash.useHeight(hashIndex + HASHENTRY_FLAG) != EMPTY &&
                boardHash.useHeight(hashIndex + HASHENTRY_64BIT1) == (boardHash.trackedHashValue ushr 32).toInt() &&
...
fun isAlwaysReplaceHashTableEntryValid(depthRemaining: Int, boardHash: BoardHash, hashIndex: Int) =
        if (boardHash.ignoreHeight(hashIndex + HASHENTRY_HEIGHT) >= depthRemaining &&
                boardHash.ignoreHeight(hashIndex + HASHENTRY_FLAG) != EMPTY &&
                boardHash.ignoreHeight(hashIndex + HASHENTRY_64BIT1) == (boardHash.trackedHashValue ushr 32).toInt() &&
