...
import com.netsensia.rivalchess.engine.board.pawnValuesCalculated
...
fun pawnScore(attacks: Attacks, board: EngineBoard): Int {
    ...
    else
    {
        ...
        VALUE_DOUBLED_PAWN_PENALTY * (board.pawnValuesCalculated(BITBOARD_WP) / 100 - popCount(whiteOccupiedFileMask)) -
        popCount(whiteOccupiedFileMask.inv() ushr 1 and whiteOccupiedFileMask) * VALUE_PAWN_ISLAND_PENALTY +
        VALUE_DOUBLED_PAWN_PENALTY * (board.pawnValuesCalculated(BITBOARD_BP) / 100 - popCount(blackOccupiedFileMask)) +
