...
public class MagicBitboards {
    ...
    public static final long[] magicNumberBishop = {
    ...
    public static final int[] magicNumberShiftsRook = {
    ...
    public static final int[] magicNumberShiftsBishop = {
    ...
    public final long[][] magicMovesRook;
    public final long[][] magicMovesBishop;

    public long[] occupancyVariation;
    public long[] occupancyAttackSet;

    public MagicBitboards() {
        magicMovesRook = new long[64][4096];
        magicMovesBishop = new long[64][1024];
        occupancyVariation = new long[4096];
    ...
    private void setMagicMoves(boolean isRook, int bitRef, int bitCount) {
        int variations;
        int i;
        long validMoves;
    ...
    private void setMagicMovesForBishop(int bitRef, int i, long validMoves) {
        int magicIndex;
        magicIndex = (int) ((occupancyVariation[i] * magicNumberBishop[bitRef]) >>> magicNumberShiftsBishop[bitRef]);
    ...
    private long setMagicMovesForSouthWestDiagonal(int bitRef, int i, long validMoves) {
        int j;
        for (j = bitRef - 7; j % 8 != 0 && j >= 0; j -= 7) {
            validMoves |= (1L << j);
            if ((occupancyVariation[i] & (1L << j)) != 0) {
                break;
            }
        }
        return validMoves;
    ...
    private long setMagicMovesForNorthEastDiagonal(int bitRef, int i, long validMoves) {
        int j;
    ...
    private long setMagicMovesForSouthEastDiagonal(int bitRef, int i, long validMoves) {
        int j;
    ...
    private long setMagicMovesForNorthWestDiagonal(int bitRef, int i, long validMoves) {
        int j;
    ...
    private void setMagicMovesForRooks(int bitRef, int i, long validMoves) {
        int magicIndex;
        int j;
        magicIndex = (int) ((occupancyVariation[i] * magicNumberRook[bitRef]) >>> magicNumberShiftsRook[bitRef]);

        for (j = bitRef + 8; j <= 63; j += 8) {
            validMoves |= (1L << j);
            if ((occupancyVariation[i] & (1L << j)) != 0) break;
        }
    ...
    private void calculateOccupancyAttackSets(boolean isRook, int bitRef, List<Integer> setBitsInMask, int bitCount) {
        int variationCount;
        int i;
        ...
        for (i = 0; i < variationCount; i++) {
            ...
            if (j >= 0 && j <= 63) {
                occupancyAttackSet[i] |= (1L << j);
            }
        }
    }

    private int calculateOccupancyAttackSetsBishop(int bitRef, int i) {
        int j;
        for (j = bitRef + 9; j % 8 != 7 && j % 8 != 0 && j <= 55 && (occupancyVariation[i] & (1L << j)) == 0; j += 9) ;
        if (j >= 0 && j <= 63) occupancyAttackSet[i] |= (1L << j);
        for (j = bitRef - 9; j % 8 != 7 && j % 8 != 0 && j >= 8 && (occupancyVariation[i] & (1L << j)) == 0; j -= 9) ;
        if (j >= 0 && j <= 63) occupancyAttackSet[i] |= (1L << j);
    ...
    private int calculateOccupancyAttackSetsRook(int bitRef, int i) {
        int j;
        for (j = bitRef + 8; j <= 55 && (occupancyVariation[i] & (1L << j)) == 0; j += 8) ;
        if (j >= 0 && j <= 63) occupancyAttackSet[i] |= (1L << j);
        for (j = bitRef - 8; j >= 8 && (occupancyVariation[i] & (1L << j)) == 0; j -= 8) ;
        if (j >= 0 && j <= 63) occupancyAttackSet[i] |= (1L << j);
